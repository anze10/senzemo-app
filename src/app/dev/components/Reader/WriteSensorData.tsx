...
export function EncodeSensorData(
  ...
  const bytePositions = parsers.map(
    (p) => p.input.byte_position + p.input.byte_length,
  );
  ...
  for (const parser of parsers) {
    ...
    console.log(
      `Processing field ${name}: value=${value}, type=${type}, position=${byte_position}, length=${byte_length}`,
    );
    ...
    try {
      ...
      if (typeof defaultValue === "number") {
        for (let i = 0; i < byte_length; i++) {
          buffer[byte_position + i] =
            (Number(defaultValue) >> (8 * (byte_length - i - 1))) & 0xff;
        }
      }
    }
  }
