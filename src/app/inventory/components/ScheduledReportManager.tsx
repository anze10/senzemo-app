    ...
    const handleSendTestReport = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setMessage({ type: 'error', text: result.error || 'Failed to send test report' });
            }
        } catch {
            setMessage({ type: 'error', text: 'Network error while sending test report' });
        } finally {
            ...
            <Card sx={{ mb: 3 }}>
                <CardHeader
                    ...
                    action={
                        <Button
                            variant="outlined"
                            startIcon={testSending ? <CircularProgress size={16} /> : <PlayArrowIcon />}
                ...
                <CardContent>
                    {scheduledReports.length === 0 ? (
                        <Alert severity="info">
                            No scheduled reports configured. Use the Email Reports tab to set up automated reports.
                        </Alert>
                    ) : (
                        <List>
                                    ...
                                    <ListItemText
                                        ...
                                        secondary={
                                            <Box>
                                                ...
                                                {report.reportStats && (
                                                    <Typography variant="body2">
                                                        {report.reportStats.totalSensors} sensors, {report.reportStats.totalComponents} components, {report.reportStats.lowStockItems} low stock
                                                    </Typography>
                                                )}
                                                {report.errorMessage && (
                                ...
                                <ListItemText
                                    ...
                                    secondary={
                                        <Box>
                                            ...
                                            {report.reportStats && (
                                                <Typography variant="body2">
                                                    Stats: {report.reportStats.totalSensors} sensors, {report.reportStats.totalComponents} components, {report.reportStats.lowStockItems} low stock
                                                </Typography>
                                            )}
                                            {report.errorMessage && (
                                                <Typography variant="body2" color="error">
