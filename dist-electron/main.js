import * as electron_1 from "electron";
import * as path_1 from "path";
import * as fs_1 from "fs";
import * as net_1 from "net";
import * as child_process_1 from "child_process";
import * as printer_server_side_1 from "./printer/printer_server_side";
...
electron_1.ipcMain.handle("printers:list-local", async () =>
  (0, printer_server_side_1.getLocalPrinters)(),
);
electron_1.ipcMain.handle("printers:print", async (_event, target, zpl) =>
  (0, printer_server_side_1.printZpl)(target, zpl),
);
...
async function startNextServer() {
  ...
  const nextBin = path_1.default.join(
    appPath,
    "node_modules",
    "next",
    "dist",
  ...
  const logPath = path_1.default.join(
    electron_1.app.getPath("userData"),
    "next.log",
  );
  (0, fs_1.writeFileSync)(
    logPath,
    `--- Nov zagon ${new Date().toISOString()} ---\n`,
  );
  ...
  nextProcess = (0, child_process_1.spawn)(
    process.execPath,
    [nextBin, "start"],
    {
      cwd: appPath,
...
electron_1.app.whenReady().then(async () => {
  electron_1.session.defaultSession.on(
    "select-serial-port",
